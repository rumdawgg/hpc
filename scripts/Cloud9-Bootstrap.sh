...
ADMIN_PW=$(aws secretsmanager get-secret-value --secret-id "${CLUSTER_NAME}-pcluster-AD" --query SecretString --output text --region "${AWS_REGION_NAME}")
export SECRET_ARN=$(aws secretsmanager describe-secret --secret-id "${CLUSTER_NAME}-pcluster-AD" --query ARN --output text --region "${AWS_REGION_NAME}")
...
/home/ec2-user/.local/bin/pcluster create-cluster --cluster-name "${CLUSTER_NAME}-pcluster" --cluster-configuration config.${AWS_REGION_NAME}.yaml --rollback-on-failure false --wait

HEADNODE_PRIVATE_IP=$(/home/ec2-user/.local/bin/pcluster describe-cluster --cluster-name "${CLUSTER_NAME}-pcluster" | jq -r '.headNode.privateIpAddress')
...
SG_HEADNODE=$(aws cloudformation describe-stack-resources --stack-name "${CLUSTER_NAME}-pcluster" --logical-resource-id ComputeSecurityGroup --query "StackResources[*].PhysicalResourceId" --output text)
...
# if [[ $FSX_ID == "AUTO" ]];then
#   FSX_ID=$(aws cloudformation describe-stack-resources --stack-name "${CLUSTER_NAME}-pcluster" --logical-resource-id FSX0 --query "StackResources[*].PhysicalResourceId" --output text)
# fi
